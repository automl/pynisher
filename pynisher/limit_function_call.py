...
def subprocess_func(func, pipe, logger, mem_in_mb, cpu_time_limit_in_s, wall_time_limit_in_s, num_procs,
    ...
    # it's tempting to use context managers here to open and close the files and also redirect
    # stdout and stderr. However, doing so would change the stderr before the exception is
    # printed to stderr and it would not show up -> the following code is therefore correct!
        ...
        class function_wrapper(object):
            ...
            def __call__(self2, *args, **kwargs):
                ...
                if self.capture_output:
                    ...
                    with open(os.path.join(tmp_dir_name, 'std.out'), 'w'):
                        pass
                    with open(os.path.join(tmp_dir_name, 'std.err'), 'w'):
                        pass
                else:
                    tmp_dir_name = None
