...
class test_limit_resources_module(unittest.TestCase):
    ...
    @unittest.skipIf(
        not all_tests, "skipping fitting an SVM to see how C libraries are handles"
    )
    ...
    def test_liblinear_svc(self):
        ...
        self.assertEqual(
            this_logger.debug.call_args_list[0][0][0],
            "Restricting your function to 2 seconds cpu time.",
        )
        self.assertEqual(
            this_logger.debug.call_args_list[1][0][0],
            "Allowing a grace period of 1 seconds.",
        )
        self.assertEqual(
            this_logger.debug.call_args_list[2][0][0],
    ...
    @unittest.skipIf(
        sys.version_info < (3, 7), "This check does not work for 3.6 python"
    )
    ...
    def test_too_little_memory(self):
        ...
        # 2048 MB
        dummy_content = [42.0] * ((1024 * 2048) // 8)  # noqa
    ...
    def test_capture_output_error(self):
        ...
        self.assertIn(
            "Cannot recover the output from", str(logger_mock.error.call_args)
        )
...
def test_out_of_memory(logger_mock, memory_limit):
    ...
    assert (
        returned_object is None
    ), f"returned_object={returned_object}/{vars(wrapped_function)}"
...
if __name__ == "__main__":
    test_out_of_memory(logger, 1024)
    # unittest.main()
